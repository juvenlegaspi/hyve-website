...
@section('title', 'Forgot Password | HYVE Workspace')
        ...
        <div class="relative flex min-h-screen items-center justify-center px-4 py-8">
            ...
            <div class="w-full max-w-[28.5rem] rounded-[1.6rem] bg-white shadow-[0_28px_80px_rgba(12,18,15,0.22)]">
                <div class="px-7 py-8 sm:px-8">
                    <p class="text-[0.76rem] font-semibold uppercase tracking-[0.16em] text-[#3f7b3d]">Account recovery</p>
                    <h1 class="mt-2 text-[1.75rem] font-semibold tracking-[-0.04em] text-[#1d1d1d]">Forgot your password?</h1>
                    <p class="mt-2 text-[0.92rem] leading-6 text-[#858585]">Enter the email connected to your HYVE account. We will send a secure reset link if the account is active.</p>

                    @if (session('status'))
                        <div class="mt-5 rounded-[1rem] border border-[#b9d8b8] bg-[#eff8ee] px-4 py-3 text-[0.82rem] leading-5 text-[#285b2a]">
                            {{ session('status') }}
                        </div>
                    @endif
                    ...
                    <p class="mt-6 text-center text-[0.92rem] text-[#a0a0a0]">
                        ...
                        <a href="{{ route('login') }}" class="font-semibold text-[#2f642f]">Back to login &rarr;</a>
                    </p>
                </div>
            </div>
        </div>
