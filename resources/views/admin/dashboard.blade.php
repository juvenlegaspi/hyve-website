...
{{--  
    ...
    <section class="mt-5 grid gap-3.5 md:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-[0.95rem] border border-[#e0e4db] bg-white px-4 py-4">
            <p class="text-[0.82rem] text-[#9d978a]">Bookings today</p>
            <strong class="mt-2 block text-[1.6rem] font-semibold leading-none text-[#132320]">{{ $bookingsToday }}</strong>
            <p class="mt-2 text-[0.78rem] text-[#3f7b3d]">
                {{ $bookingsDelta >= 0 ? '+' : '' }}{{ $bookingsDelta }} from yesterday
            </p>
        </article>

        <article class="rounded-[0.95rem] border border-[#e0e4db] bg-white px-4 py-4">
            <p class="text-[0.82rem] text-[#9d978a]">Revenue today</p>
            <strong class="mt-2 block text-[1.6rem] font-semibold leading-none text-[#132320]">&#8369;{{ number_format($revenueToday, 0) }}</strong>
            <p class="mt-2 text-[0.78rem] text-[#3f7b3d]">{{ $paidToday }} paid &middot; {{ $pendingToday }} pending</p>
        </article>

        <article class="rounded-[0.95rem] border border-[#e0e4db] bg-white px-4 py-4">
            <p class="text-[0.82rem] text-[#9d978a]">Active members</p>
            <strong class="mt-2 block text-[1.6rem] font-semibold leading-none text-[#132320]">{{ $memberCount }}</strong>
            <p class="mt-2 text-[0.78rem] text-[#3f7b3d]">+{{ $newMembersThisWeek }} this week</p>
        </article>

        <article class="rounded-[0.95rem] border border-[#e0e4db] bg-white px-4 py-4">
            <p class="text-[0.82rem] text-[#9d978a]">Room utilization</p>
            <strong class="mt-2 block text-[1.6rem] font-semibold leading-none text-[#132320]">{{ $utilization }}%</strong>
            <p class="mt-2 text-[0.78rem] text-[#3f7b3d]">{{ $occupiedCount }} of {{ $roomCount }} rooms active now</p>
        </article>
    </section>
    ...
    <section class="mt-4 grid items-start gap-3.5 xl:grid-cols-[1.62fr_1fr]">
        <article class="grid h-[26.8rem] grid-rows-[auto_minmax(0,1fr)] overflow-hidden rounded-[0.95rem] border border-[#e0e4db] bg-white px-5 py-4.5">
            <div class="flex items-center justify-between gap-3">
                <h2 class="text-[1rem] font-semibold tracking-[-0.03em] text-[#132320]">Bookings</h2>
                <span class="text-[0.8rem] text-[#aaa398]">Live list</span>
            </div>
                        ...
                        @forelse ($recentBookings as $detail)
                            <tr>
                                ...
                                <td class="py-2.5 pr-3">
                                    <span class="@if (($detail->bookingHeader?->status ?? 'pending') === 'confirmed') bg-[#eef8df] text-[#3f6a34] @elseif (($detail->bookingHeader?->status ?? 'pending') === 'cancelled') bg-[#fde9e5] text-[#b14635] @else bg-[#f8efdd] text-[#9a7832] @endif inline-flex rounded-full px-2.5 py-0.75 text-[0.66rem] font-semibold">
                                        {{ ucfirst((string) ($detail->bookingHeader?->status ?? 'pending')) }}
                                    </span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="py-6 text-center text-[#8f897e]">No bookings yet today.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <article class="grid h-[26.8rem] grid-rows-[auto_auto_minmax(0,1fr)] overflow-hidden rounded-[0.95rem] border border-[#e0e4db] bg-white px-5 py-4.5">
            <div class="flex items-center justify-between gap-3">
                ...
                <span class="text-[0.8rem] text-[#aaa398]">As of {{ now()->format('g:i A') }}</span>
            </div>
            ...
            <div class="mt-4 min-h-0 grid gap-2.5 overflow-y-auto pr-1 sm:grid-cols-2">
                @foreach ($roomStatus as $room)
                    <div class="@if ($room['status'] === 'available') border-[#abd164] bg-[#f3fae7] @else border-[#e2e3dc] bg-white @endif rounded-[0.9rem] border p-3.5">
                        <p class="text-[0.88rem] font-semibold text-[#163128]">{{ $room['room_name'] }}</p>
                        <p class="mt-1.5 text-[0.82rem] font-semibold @if ($room['status'] === 'available') text-[#3f7b3d] @else text-[#db8d1e] @endif">
                            {{ $room['status_label'] }}
                        </p>
